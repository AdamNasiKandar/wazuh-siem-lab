...
Cross-service capstone — simulate a suspicious action with a throwaway IAM user, trace it through the full chain (CloudTrail → S3 → Wazuh ingestion → your custom rule alerting) — strongest single demo for an interview, proves you understand the whole pipeline
